...
const removeContact = async (contactId) => {
  ...
  await fs.writeFile(
    contactsPath,
    JSON.stringify(remainingСontacts),
    (error) => {
      if (error) {
        throw error;
      }
  ...
  console.log("Удаленный контакт: ", removeContact);
};
...
const addContact = async (name, email, phone) => {
  ...
  await fs.writeFile(contactsPath, JSON.stringify(responseData), (error) => {
    if (error) {
      throw error;
    }
  });

  console.log("Контакт:", newContact);
};
